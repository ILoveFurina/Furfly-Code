# system-prompt-architecture Specification

## Purpose

system-prompt-architecture 规格定义 Furfly Code 内置系统提示的结构与拼装约束：七模块固定优先级拼装、模块去规定化表述、动静分离与双缓存断点、缓存机制双轨、工具硬约束单一事实来源、缓存可观测性断言。
