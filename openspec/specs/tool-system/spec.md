...
## Purpose
tool-system 规格定义 Furfly Code 的工具系统：工具抽象与注册中心、6 个内置工具（读 / 写 / 改文件、执行命令、查找 / 搜索代码）、工具定义注入、流式工具调用解析、工具执行与超时、结果回灌与编排、界面工具行呈现、结构化错误与体量控制。
## Requirements
...
流式接收回复时，系统 SHALL 正确识别模型发起的工具调用：拼接分片到达的工具名与 JSON 参数碎片，组装出完整的工具调用请求（工具名 + 参数对象）。正文文本增量、思考增量、工具调用三者 MUST 正确区分（思考增量沿用 chat-client 的接收即丢弃）。模型一次回复中可请求一个或多个工具调用。
...
系统 SHALL 将模型的工具调用与对应执行结果按协议格式追加进对话历史，再次发起请求，让模型基于工具结果生成最终文本答复。本轮 MUST 在这次最终答复后结束——即使模型在最终答复阶段又想调用工具，也不再发起新一轮工具执行。
...
命令执行与潜在长耗时的工具 MUST 受超时约束；超时即终止该工具并返回超时结果，界面 SHALL 不冻结、会话 SHALL 不挂死。超时为内置合理默认值，暂不通过配置调整。
...
工具执行与流式期间，界面 MUST 保持响应（可滚动、可见进行中指示），不冻结（沿用 chat-client 的非阻塞约束）。
...
工具调用的触发、UI 展示、结果回灌、错误反馈，在 Anthropic 与 OpenAI 两种协议下 SHALL 行为一致（沿用并扩展 chat-client 的跨协议一致约束）。
...
工具参数缺失/类型错误、路径非法、命令报错、JSON 参数解析失败等，均 MUST 以结构化错误处理，绝不抛未捕获异常或崩溃堆栈（沿用并扩展 chat-client 的健壮性约束）。
...
API 密钥 MUST 不回显、不打印到对话区或任何输出中（沿用 chat-client 的密钥安全约束）。
