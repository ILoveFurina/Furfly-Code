...
- [x] read_file 带行号读出内容；读不存在/目录返回结构化错误（验证：单测 + 手测读 `docs/features/step2.工具系统/spec.md` 见行号、读不存在文件得 `is_error`）。(AC2/F2)
...
- [x] 单轮闭环端到端：问「读 X 并总结」→ 模型调用 read_file → 结果回灌 → 给出最终文本总结（验证：`python -m furflycode` 跑通，答复体现文件内容）。(AC8/F5/F6)
...
- [x] 场景 2（单轮内多工具顺序执行）：让模型「读 spec.md 和 plan.md 两个文件并对比要点」→ 若模型一次请求返回两个 read_file 调用，观察两个工具行依次出现、结果正确（F5 顺序执行）；若模型串行（先一个、续答再一个），续答的第二次工具调用被 AC9 忽略属预期行为，非 bug。write_file / bash 各自单工具回合不受单轮上限影响，可单独手测覆盖。
