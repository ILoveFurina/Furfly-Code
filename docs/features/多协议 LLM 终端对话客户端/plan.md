...
3. LLM 协议层 `furflycode.llm` —— 定义协议无关的 `Provider` Protocol 与统一消息/流式事件类型；
...
6. 终端层 `furflycode.tui` —— Textual App，含状态机（选择/空闲/流式）、输入框、对话区、
...
## 模块设计
### 模块 `furflycode.config`
职责：读取并校验 `.furflycode/config.yaml`，产出 providers 列表。
...
对外接口：`furflycodeApp(providers: list[ProviderConfig])`；`App.run()` 或 `await App.run_async()`。
内部职责：
...
### 模块 `furflycode.cli`（入口）
职责：装配与启动。
流程：`config.load(...)` → `prompt.render_banner(version, cwd)` 打印 → `furflycodeApp(cfg.providers).run()`。
...
### 调用链（启动）
```
main() → config.load(".furflycode/config.yaml")
       → 若 ConfigError：打印可读错误、sys.exit(1)
       → print(prompt.render_banner(version, cwd))
       → furflycodeApp(cfg.providers).run()
...
Provider.stream ──async generator──> StreamEvent (text / done / err)
                                         │
                                         └──async for──> furflycodeApp._consume_stream
                                                            │
                                                            ├── text  → 动态区追加
...
│   └── furflycode/
│       ├── __init__.py
│       ├── __main__.py             — 允许 `python -m furflycode`
...
- `.furflycode/config.yaml` 含真实密钥，应在 `.gitignore` 忽略；提交一份 `config.yaml.example`。
- `pyproject.toml` 里通过 `[project.scripts] furflycode = "furflycode.cli:main"` 暴露 CLI 入口；
  装好后既可 `furflycode` 也可 `python -m furflycode`。

## 技术决策

| 决策点 | 选择 | 理由 |
|--------|------|------|
| 语言 | Python 3.12+ | 3.12 的 typing/`asyncio.TaskGroup` 等更舒服 |
...
| 配置 | `.furflycode/config.yaml` + `pyyaml`；密钥入 `.gitignore` | 用户既定路径；N5 密钥安全 |
