...
> chat-client 与 tool-system 是新 capability，全用 `ADDED`。下面 agent loop 会用到 `MODIFIED`（把"单轮闭环"改成"多轮 loop"）。
...
- **你说**：`/opsx:propose 加 agent loop：把 tool-system 的单轮闭环改成多轮工具循环，模型拿工具结果后可继续调用，直到给出最终答复或达循环上限。change 名叫 agent-loop，修改 tool-system capability。`
...
> 背景：tool-system spec 里「结果回灌与单轮闭环」那条写的是"单轮闭环……不做连环调用"。agent loop 就是把这条改成"多轮循环"。这是个**修改既有 capability** 的例子，用到 `MODIFIED Requirements`，比之前全 `ADDED` 的 chat-client/tool-system 更进一步。

### 5.1 决定 capability 归属
agent loop 改的是 tool-system 的编排行为（单轮→多轮），所以：
- **Modified Capabilities**：`tool-system`（改它的「结果回灌与单轮闭环」那条，可能再加几条新需求如"循环上限""终止条件"）。
- 不是新 capability（不建 `specs/agent-loop/`），而是改 `specs/tool-system/spec.md`。

### 5.2 你对 Claude 说的话（按顺序）

**① 立提案**
> `/opsx:propose 加 agent loop：把 tool-system 的单轮闭环改成多轮工具循环，模型拿工具结果后可继续调用工具，直到给出最终答复或达到循环上限。change 名叫 agent-loop，修改 tool-system capability。`
...
3. **纯重构没有 spec delta 怎么归档**：`spec-driven` 要求 change 至少有一个 delta，否则校验报 `No deltas found`。行为零变化的纯重构（如 refactor-tool-types 类型归位）归档时跟 Claude 说"这是纯重构，归档时跳过 spec 同步和校验"，让它用：
   ```
...
7. **改代码 ≠ 改 spec**：只动实现、行为没变（重构、性能优化），不要碰 spec，走 refactor-tool-types 那种 spec-less 归档。spec 只记"系统该做什么"的**行为契约**。
